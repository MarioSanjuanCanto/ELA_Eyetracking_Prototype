...
import { cloneVoiceAction, getClonedVoicesAction, testVoiceAction } from "@/lib/voice-actions";
...
export function RecordScreen({
  ...
  const handleTestVoice = async () => {
    try {
      const result = await testVoiceAction({
        data: "¡Hola, buenos días, me llamo Fran y tengo la enfermedad ELA y necesito que alguien me preste atención, gracias! ",
      });
    ...
    <div className="relative flex w-full flex-col items-center">
      ...
      <div className="mt-8 flex w-full max-w-5xl justify-end gap-4">
        ...
        {isAdmin && (
          <button
            onClick={onContinue}
            className="rounded-full border border-border bg-background px-10 py-4 text-lg font-semibold text-foreground shadow-sm transition hover:bg-muted"
          >
            {t.continue}
          </button>
        )}
