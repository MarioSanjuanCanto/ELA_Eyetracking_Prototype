...
export function StartScreen({ t, onStart }: { t: OnboardingTexts; onStart: () => void }) {
  return (
    <div className="flex w-full flex-col items-center">
      ...
      <div className="mt-12 grid w-full max-w-[1100px] gap-6 md:grid-cols-3">
        <FeatureCard icon={User} title={t.personalizedTitle} desc={t.personalizedDesc} />
        <FeatureCard icon={ShieldCheck} title={t.secureTitle} desc={t.secureDesc} />
        <FeatureCard icon={Sparkles} title={t.aiPoweredTitle} desc={t.aiPoweredDesc} />
      </div>
    </div>
  );
}
