...
function Index() {
  const [step, setStep] = useState<"start" | "name" | "record" | "eyeTracking">("start");
      ...
      {step !== "eyeTracking" && <PanelShell>
        ...
        {step === "record" && (
          <RecordScreen
            ...
            isAdmin={isAdmin}
            onBack={() => setStep("name")}
            onCloneSuccess={() => setStep("eyeTracking")}
            onContinue={() => setStep("eyeTracking")}
          />
        )}
      </PanelShell>}
      {step === "eyeTracking" && (
